...
@media (prefers-color-scheme: dark) {
    ...
    .stButton > button:hover { background: #E5E5EA !important; }
...
AI_GAINS_MISSES = {
    ...
    "Research Support": [
        ("Summarises funder eligibility rules and deadlines", "Cannot judge which funders are realistically likely to say yes"),
...
{{
  ...
  "hidden_contribution": "1-2 sentences on a contribution this role makes that rarely gets credited in research outputs"
}}
...
- All four fields are required — do not omit hidden_contribution"""
...
if st.session_state.translated and st.session_state.current_idea:
    ...
    if result:
        with st.container():
            ...
            # Hidden contribution
            hidden = result.get("hidden_contribution", "").strip()
            if not hidden:
                try:
                    retry = generate_perspective(st.session_state.current_idea, perspective)
                    hidden = retry.get("hidden_contribution", "").strip()
                    result["hidden_contribution"] = hidden
                    st.session_state.perspective_cache[perspective] = result
                except Exception:
                    hidden = ""

            st.markdown("<div class='ios-section-label'>Hidden contribution</div>", unsafe_allow_html=True)
            st.markdown(
                f"<div class='ios-card'>{hidden if hidden else 'Not available — please try translating again.'}</div>",
                unsafe_allow_html=True
            )

            # Why this voice is often quiet
            st.markdown("<div class='ios-section-label'>Why this voice is often quiet</div>", unsafe_allow_html=True)
            st.markdown(f"<div class='ios-card' style='font-style:italic;'>{cfg['quiet_reason']}</div>", unsafe_allow_html=True)
...
This prototype asks: what happens when we try to make them heard?</strong>
</p>
</div>
""", unsafe_allow_html=True)
